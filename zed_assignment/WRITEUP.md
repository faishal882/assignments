...
The file-backed adapter builds Shapely spatial indexes and performs a bbox candidate query before exact projected intersection. At county scale, normalized inputs should move to PostGIS with GiST indexes. Complexity is driven primarily by vertex count in union/difference operations, so complex flood boundaries will dominate more than the raw layer count. County-wide batch scoring is a separate workload and should precompute or cache parcel/constraint intersections.
...
SQLite plus R-tree indexes is deliberate for a self-contained take-home and handles a moderate county without scanning every geometry. Full multi-county production would move the same repository contract to PostGIS, use GiST indexes and vector tiles, run expensive overlays as cancellable jobs, and version source snapshots. Production work also requires source-license review, current FEMA/NWI classification filters, and ingestion observability. A permitting workflow would additionally require restore audit identity, evidence, and approval state.
